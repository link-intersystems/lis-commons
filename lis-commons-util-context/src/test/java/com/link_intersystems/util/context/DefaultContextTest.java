...
class DefaultContextTest {
    ...
    void stream() {
        ...
        List<?> instances = context.stream().map(QualifiedObject::getObject).collect(Collectors.toList());
        assertEquals(2, instances.size());
        assertTrue(instances.contains("A"));
        assertTrue(instances.contains("B"));

        context.stream().map(QualifiedObject::getQualifier).collect(Collectors.toList()).forEach(context::remove);
    }
}