...
public class MemberCriteria<T extends Member> extends ElementCriteria<T> {
    ...
    public void setMemberIterateOrder(Comparator<Member> iterateOrderComparator) {
        this.iterateOrderComparator = requireNonNull(iterateOrderComparator);
    }
    ...
    protected Iterator<T> applyAccessAndNamePredicates(Iterator<T> iterator) {
        ...
        Predicate<Member> accessModifierPredicate = new MemberModifierPredicate(accessModifiers, MemberModifierPredicate.Match.AT_LEAST_ONE);

        if (accesses.contains(AccessType.DEFAULT)) {
            MemberModifierPredicate memberModifierPredicate = new MemberModifierPredicate(Modifier.PRIVATE | Modifier.PUBLIC | Modifier.PROTECTED, MemberModifierPredicate.Match.AT_LEAST_ONE);
        ...
        if (modifiers != 0) {
            predicates.add((Predicate<T>) new MemberModifierPredicate(modifiers, MemberModifierPredicate.Match.AT_LEAST_ALL));
        }
