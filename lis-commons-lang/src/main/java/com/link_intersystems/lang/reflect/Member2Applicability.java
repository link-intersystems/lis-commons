...
class Member2Applicability implements Serializable {
	...
	public boolean isApplicable(Class<?>[] paramTypes) {
		...
		boolean applicable = phase1CheckApplicableSubtype(paramTypes,
				parameters);
		if (applicable) {
			return true;
		}

		applicable = phase2CheckMethodInvocationConversion(paramTypes,
				parameters);
		if (applicable) {
			return true;
		}
	 ...
	 * Subtyping
	 */
	private boolean phase1CheckApplicableSubtype(Class<?>[] paramTypes,
		...
		for (int i = 0; i < parameters.size(); i++) {
			...
			if (!parameterInfo.isApplicableSubtype(invocationParameter)
					&& !parameterInfo
							.isApplicableAutobixing(invocationParameter)) {
				applicable = false;
				break;
	 ...
	 */
	private boolean phase2CheckMethodInvocationConversion(
			Class<?>[] paramTypes, List<Parameter> parameters) {
		boolean applicable;
		applicable = true;
		for (int i = 0; i < parameters.size(); i++) {
			Parameter parameterInfo = parameters.get(i);
			Class<?> invocationParameter = paramTypes[i];
			if (!parameterInfo
					.isApplicableMethodInvocationConversion(invocationParameter)) {
				applicable = false;
				break;
			}
		}
	 ...
	 */
	private boolean phase3CheckApplicableVariableArityMethods(
			Class<?>[] paramTypes) {
		boolean applicable;
		applicable = true;
		for (int i = 0; i < paramTypes.length; i++) {
			...
			Class<?> invocationParameter = paramTypes[i];
			if (!param
					.isApplicableVariableArityMethodInvocationConversion(invocationParameter)) {
				applicable = false;
				break;
			}
		}
