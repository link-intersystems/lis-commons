...
public abstract class ReflectFacade {
	...
	private static final class SerializableReferenceImplementation implements
			SerializableReference<Predicate<Class<?>>> {
		/**
		 *
		 */
	...
	static {
		CLASS_IS_INTERFACE_PREDICATE = new SerializablePredicate<Class<?>>(
				new SerializableReferenceImplementation());
	}
	...
	public static Predicate getMemberNamePredicate(String memberName) {
		...
		class SerializableMemberNamePredicate implements
				SerializableReference<Predicate> {
			...
			public Predicate get() {
				if (predicate == null) {
					Method memberGetNameMethod = MethodUtils
							.getAccessibleMethod(Member.class, "getName",
									new Class<?>[0]);
					...
					Predicate propertyValuePredicate = AndPredicate
							.andPredicate(NotNullPredicate.notNullPredicate(),
									new EqualPredicate(memberName));
					Predicate instance = TransformedPredicate
							.transformedPredicate(member2NameTransformer,
									propertyValuePredicate);
					Predicate memberNamePredicate = AndPredicate.andPredicate(
							NotNullPredicate.notNullPredicate(), instance);
		...
		return new SerializablePredicate(new SerializableMemberNamePredicate(
				memberName));
	}
	...
	public static Predicate getDeclaringClassPredicate(Class<?> declaringClass) {
		...
		class SerializableDeclaringClassPredicate implements
				SerializableReference<Predicate> {
			...
			public Predicate get() {
				if (predicate == null) {
					Method getDeclaringClassMethod = MethodUtils
							.getAccessibleMethod(Member.class,
									"getDeclaringClass", new Class<?>[0]);
					MethodInvokingTransformer member2NameTransformer = new MethodInvokingTransformer(
							getDeclaringClassMethod);
					Predicate propertyValuePredicate = AndPredicate
							.andPredicate(NotNullPredicate.notNullPredicate(),
									new EqualPredicate(declaringClass));
					Predicate instance = TransformedPredicate.transformedPredicate(
							member2NameTransformer, propertyValuePredicate);
					Predicate memberNamePredicate = AndPredicate.andPredicate(
							NotNullPredicate.notNullPredicate(), instance);
					this.predicate = memberNamePredicate;
				}
		...
		return new SerializablePredicate(
				new SerializableDeclaringClassPredicate(declaringClass));
	}
	...
	/**
	 * A {@link Transformer} that transforms a {@link Field} into it's type.
	 *
	 * @return a {@link Transformer} that transforms a {@link Field} into it's
	 *         type.
	 * @since 1.0.0.0
	 */
	public static Transformer getFieldTypeTransformer() {
		return FieldTypeTransformer.getInstance();
	}

	/**
	 * A {@link Transformer} that transforms a {@link Method} into it's return
	 * type.
	 *
	 * @return a {@link Transformer} that transforms a {@link Method} into it's
	 *         return type.
	 * @since 1.0.0.0
	 */
	public static Transformer getMethodTypeTransformer() {
		return MethodTypeTransformer.getInstance();
	}
	 ...
	 * @since 1.0.0.0
	 */
	public static SerializableReference<? extends Member> getSerializableReference(
			Member member) {
		if (member instanceof Method) {
			...
			throw new IllegalArgumentException(
					"member must be one of the type " + Constructor.class
							+ ", " + Method.class + " or " + Field.class);
		}
	}
	 ...
	 * @since 1.0.0.0
	 */
	public static SerializableReference<Constructor<?>> getSerializableReference(
	 ...
	 * @since 1.0.0.0
	 */
	public static SerializableReference<Method> getSerializableReference(
			Method method) {
	 ...
	 * @since 1.0.0.0
	 */
	public static SerializableReference<Field> getSerializableReference(
			Field field) {
	 ...
	 * @since 1.0.0.0
	 */
	public static SerializableReference<Package> getSerializableReference(
			Package packageObj) {
...
/**
 * A {@link Transformer} that transforms a {@link Method} into it's return type.
 *
 * @author René Link <a
 *         href="mailto:rene.link@link-intersystems.com">[rene.link@link-
 *         intersystems.com]</a>
 * @since 1.0.0.0
 */
class MethodTypeTransformer implements Transformer, Serializable {
 ...
 * @since 1.0.0.0
 */
class FieldTypeTransformer implements Transformer, Serializable {
...
class MemberNamePatternPredicate implements Predicate, Serializable {
	 ...
	 *
	 * @param namePattern
	 *            the {@link Pattern} to use when evaluating.
	 * @since 1.0.0.0
	 */
	...
	public boolean evaluate(Object object) {
		if (!Member.class.isInstance(object)) {
			throw new IllegalArgumentException(
					MemberNamePatternPredicate.class.getSimpleName()
							+ " can only handle " + Member.class);
		}
...
class AssignablePredicate implements Predicate, Serializable {
	 ...
	 *
	 * @param object
	 *            must be a {@link Class}. Other objects always evaluate to
	 *            false.
	 * @since 1.0.0.0
	 */
	public boolean evaluate(Object object) {
 ...
 * @since 1.0.0.0
 */
class CanonicalClassNameComparator implements Comparator<Class<?>>,
		Serializable {
...
class SerializablePredicate<T> implements Predicate<T>, Serializable {
	...
	public SerializablePredicate(
			SerializableReference<Predicate<T>> serializablePredicate) {
